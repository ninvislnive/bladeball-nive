-- Nive Blade Ball NO LAG EDITION (Xeno optimized)
-- Много функций, космическое меню, чёрная дыра, без лагов
...
spawn(function()
    ...
    for _=1,20 do  -- меньше частиц, чтобы не лагало
...
local tabNames = {"Main","Combat","Visual","Defense","Teleport","Fun","Server","Settings","Stats","Credits"}
local tabBtns = {}
local contents = {}
...
cred.Text = "Nive Blade Ball\nCreated by Nive\nDonationAlerts: /r/nive"
...
-- ================= ЧЁРНАЯ ДЫРА (СКРЫТИЕ МЕНЮ ПО ALT) =================
    ...
    if input.KeyCode == Enum.KeyCode.RightAlt then
        ...
        if Settings.MenuOpen then
            ...
            TweenService:Create(main, TweenInfo.new(0.3, Enum.EasingStyle.Back), {
                BackgroundTransparency = 0, Position = UDim2.new(0,20,0,20)
            }):Play()
        else
            blackHole.Size = UDim2.new(0,0,0,0)
            ...
            TweenService:Create(main, TweenInfo.new(0.2), {
                BackgroundTransparency = 1, Position = UDim2.new(0,20,0,50)
            }):Play()
            task.wait(0.2)
            main.Visible = false
        end
...
-- ================= УТИЛИТЫ =================
function getRoot() return LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") end
function getHum() return LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") end
function canAct()
    if not Settings.AntiBan then return true end
    if tick() - Settings.LastAction >= Settings.ActionDelay then Settings.LastAction = tick(); return true end
    return false
end
function findBall()
...
local function autoParry()
    ...
    if (root.Position - ball.Position).Magnitude < Settings.ParryRadius then
...
RunService.Heartbeat:Connect(function()
    pcall(autoParry); pcall(autoBlock); pcall(flight); pcall(godMode); pcall(noclip); pcall(esp)
...
print("Nive Blade Ball NO LAG загружен!")
